...
public class OthelloBoard {
	...
	private char checkForPattern(int row, int col, int drow, int dcol) {
		...
		if (this.getToken(row, col) == EMPTY || (drow == 0 && dcol ==0)) {
			return EMPTY;
		}
		char tokens = this.getToken(row, col);
		int r = row + drow;
		int c = col + dcol;
		while (this.validCoordinate(r, c)) {
			if (this.getToken(r, c) != tokens)  {
				return this.getToken(r, c);
			}else {
				r = r + drow;
				c = c + dcol;
			}
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		if (this.checkForPattern(row, col, drow, dcol) == player) {
			...
			int r = row;
			int c = col;
			while (this.getToken(r, c) != player) {
		...
		if (this.checkForPattern(row, col, drow, dcol) != EMPTY) {
			return 0;
		}
		return -1;
	}
	...
	public boolean move(int row, int col, char player) {
		...
		if (this.getToken(row, col) != EMPTY) {
			return false;
		}
