...
public class Othello {
	...
	public boolean move(int row, int col) {
		...
		char next = OthelloBoard.otherPlayer(this.whoseTurn);
		if (board.hasMove() != next && board.move(row, col, this.whoseTurn)) {
			this.numMoves ++;
			if (board.getCount(next) == 0) {
				this.whoseTurn = OthelloBoard.EMPTY;
				return true;
			}
	...
	public char getWinner() {
		...
		int p1_count = board.getCount(OthelloBoard.P1);
		int p2_count =  board.getCount(OthelloBoard.P2);
		if (this.isGameOver() && (p1_count != p2_count)) {
